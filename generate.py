...
assert os.path.isfile(sess_path), '"{' + str(sess_path) + '}" is not a file'
...
if control is not None:
    if os.path.isfile(control) or os.path.isdir(control):
        if os.path.isdir(control):
            ...
            assert len(files) > 0, 'no file in "{' + str(control) + '}"'
        ...
        control = 'control sequence from "{' + str(control) + '}"'
